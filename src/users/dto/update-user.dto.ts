...
export class UpdateUserDto {
    @ApiProperty({ example: 'user@gmail.com', description: 'will checked by unique' })
    @IsOptional()
    ...
    @IsOptional()
    @IsString({ message: 'STRING field type expected' })
    @Length(FULLNAME_MINIMAL_LENGTH, FULLNAME_MAXIMUM_LENGTH, { message: `User fullname must be from ${FULLNAME_MINIMAL_LENGTH} to ${FULLNAME_MAXIMUM_LENGTH} characters` })
